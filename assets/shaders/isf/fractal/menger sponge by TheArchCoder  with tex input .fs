/*
{
  "DESCRIPTION": "Menger Sponge Fractal converted to ISF, with controls for Ambient Occlusion, Vignette, Glow, Fog, Shadows, Lighting, Rendering, Coloring, Refraction, and new manual fractal/camera controls, including image textures.",
  "CATEGORIES": [ "Fractal", "Menger Sponge", "3D", "Procedural", "Abstract", "Texture" ],
  "ISF_VERSION": "2.0",
  "INPUTS": [
    ...
    { "NAME": "iterations", "TYPE": "float", "DEFAULT": 12.0, "MIN": 1.0, "MAX": 50.0, "STEP": 1.0, "DESCRIPTION": "Number of fractal iterations." },
    ...
    { "NAME": "raystep_multiplier", "TYPE": "float", "DEFAULT": 0.6, "MIN": 0.1, "MAX": 2.0, "DESCRIPTION": "Multiplier for raymarch step size." },
    ...
    { "NAME": "palette_color1_g", "TYPE": "float", "DEFAULT": 0.0, "MIN": 0.0, "MAX": 1.0, "DESCRIPTION": "Green component of first palette color." },
    { "NAME": "palette_color1_b", "TYPE": "float", "DEFAULT": 0.0, "MIN": 0.0, "MAX": 1.0, "DESCRIPTION": "Blue component of first palette color." },
    ...
    { "NAME": "mengersponge_offset_x", "TYPE": "float", "DEFAULT": 1.3, "MIN": -5.0, "MAX": 5.0, "DESCRIPTION": "X offset for Menger Sponge transformation." },
    { "NAME": "mengersponge_offset_y", "TYPE": "float", "DEFAULT": 1.3, "MIN": -5.0, "MAX": 5.0, "DESCRIPTION": "Y offset for Menger Sponge transformation." },
    { "NAME": "mengersponge_offset_z", "TYPE": "float", "DEFAULT": 1.3, "MIN": -5.0, "MAX": 5.0, "DESCRIPTION": "Z offset for Menger Sponge transformation." },
    { "NAME": "mengersponge_rotation_alpha", "TYPE": "float", "DEFAULT": 0.0, "MIN": -3.14, "MAX": 3.14, "DESCRIPTION": "Alpha rotation for Menger Sponge (Y-axis)." },
    { "NAME": "mengersponge_rotation_beta", "TYPE": "float", "DEFAULT": 0.0, "MIN": -3.14, "MAX": 3.14, "DESCRIPTION": "Beta rotation for Menger Sponge (X-axis)." },
    { "NAME": "mengersponge_mouse_toggle", "TYPE": "float", "DEFAULT": 0.0, "MIN": 0.0, "MAX": 1.0, "STEP": 1.0, "DESCRIPTION": "Toggle between default offset (0) and custom offset (1) for Menger Sponge." },
...
// Menger Sponge SDF function - directly from your code, adapted for ISF inputs
vec2 mengersponge_sdf(vec3 p) {
    // All these variables are implicitly passed from the JSON INPUTS as floats
    // float fractal_power;
    // float mengersponge_offset_x, mengersponge_offset_y, mengersponge_offset_z;
    // float iterations;
    // float mengersponge_rotation_alpha, mengersponge_rotation_beta;
    // float mengersponge_mouse_toggle;

float scale = fractal_power + 1.0; // Use fractal_power input

    vec3 offset;
    if (mengersponge_mouse_toggle > 0.5) { // Check toggle as float
        // Reconstruct mouse-like offset from sliders
        // Original: vec3(2.0 * iMouse.xy / iResolution.xy, 1.0)
        // Assuming mengersponge_offset_x/y are now normalized 0-1 values if toggle is on
        offset = vec3(2.0 * mengersponge_offset_x, 2.0 * mengersponge_offset_y, 1.0); 
    } else {
        offset = vec3(mengersponge_offset_x, mengersponge_offset_y, mengersponge_offset_z);
    }

float colorind = 0.0; // This variable is declared but not used in the original SDF, keeping it for consistency
 float alpha = mengersponge_rotation_alpha; // Use input for alpha rotation
    float beta = mengersponge_rotation_beta; // Use input for beta rotation
 ...
 p *= sphereFold(vec4(p, 0.0), 0.0, 1.2, 1.0);
 p.xyz = boxFold(vec4(p, 0.0), vec3(0.2)).xyz;
 p = abs(p) - vec3(0, 1.4, 0);
...
p -= (offset * vec3(1, 1, 0)) * (scale - 1.0);
}
 }

 float d = length(p) * pow(scale, -float(i));
 return vec2(orbit_trap / 2.3, d);
}
...
vec3 ray_marcher(vec3 ro, vec3 rd) {
	...
	for (int i = 0; i < int(max_steps); i++) { // Cast max_steps to int
		vec2 data = mengersponge_sdf(ro + rd * dfo);
...
float soft_shadow(vec3 p, vec3 light_pos, float k) {
	...
	for (int i = 0; i < int(shadow_steps); i++) { // Cast shadow_steps to int
		float h = mengersponge_sdf(p + rd * t).y;
...
vec3 calculate_normal(vec3 p) {
	...
	return normalize(vec3(
		mengersponge_sdf(p + vec3(h, 0.0, 0.0)).y - mengersponge_sdf(p - vec3(h, 0.0, 0.0)).y,
		mengersponge_sdf(p + vec3(0.0, h, 0.0)).y - mengersponge_sdf(p - vec3(0.0, h, 0.0)).y,
		mengersponge_sdf(p + vec3(0.0, 0.0, h)).y - mengersponge_sdf(p - vec3(0.0, 0.0, h)).y
	));
}
...
float calculate_ambient_occlusion(vec3 p, vec3 normal) {
	...
	for (int i = 0; i < int(ambient_occlusion_steps); i++) { // Cast to int
		...
		float d = mengersponge_sdf(sample_point).y;
...
void main() { // ISF entry point
    ...
    vec3 ro = vec3(camera_base_x, camera_base_y, camera_base_z + camera_amplitude * sin(camera_frequency * time + camera_phase) ); 
