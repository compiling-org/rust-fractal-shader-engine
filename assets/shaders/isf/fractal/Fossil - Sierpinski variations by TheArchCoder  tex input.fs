/*
{
  "DESCRIPTION": "Sierpinski Octahedron Fractal converted to ISF, with controls for Ambient Occlusion, Vignette, Glow, Fog, Shadows, Lighting, Rendering, Coloring, Refraction, and new manual fractal/camera controls, including image textures.",
  "CATEGORIES": [ "Fractal", "Sierpinski", "Octahedron", "3D", "Procedural", "Abstract", "Texture" ],
  "ISF_VERSION": "2.0",
  "INPUTS": [
    ...
    { "NAME": "palette_color1_r", "TYPE": "float", "DEFAULT": 0.8, "MIN": 0.0, "MAX": 1.0, "DESCRIPTION": "Red component of first palette color." },
    { "NAME": "palette_color1_g", "TYPE": "float", "DEFAULT": 0.3, "MIN": 0.0, "MAX": 1.0, "DESCRIPTION": "Green component of first palette color." },
    { "NAME": "palette_color1_b", "TYPE": "float", "DEFAULT": 0.1, "MIN": 0.0, "MAX": 1.0, "DESCRIPTION": "Blue component of first palette color." },
    { "NAME": "palette_color2_r", "TYPE": "float", "DEFAULT": 2.0, "MIN": 0.0, "MAX": 2.0, "DESCRIPTION": "Red component of second palette color." },
    ...
    { "NAME": "sierpinski_offset_x", "TYPE": "float", "DEFAULT": 0.3, "MIN": -5.0, "MAX": 5.0, "DESCRIPTION": "X offset for Sierpinski Octahedron transformation." },
    { "NAME": "sierpinski_offset_y", "TYPE": "float", "DEFAULT": 0.7, "MIN": -5.0, "MAX": 5.0, "DESCRIPTION": "Y offset for Sierpinski Octahedron transformation." },
    { "NAME": "sierpinski_offset_z", "TYPE": "float", "DEFAULT": 0.8, "MIN": -5.0, "MAX": 5.0, "DESCRIPTION": "Z offset for Sierpinski Octahedron transformation." },
    { "NAME": "sierpinski_mouse_toggle", "TYPE": "float", "DEFAULT": 0.0, "MIN": 0.0, "MAX": 1.0, "STEP": 1.0, "DESCRIPTION": "Toggle between default offset (0) and custom offset (1)." },
    { "NAME": "sierpinski_rotation_x", "TYPE": "float", "DEFAULT": 1.0, "MIN": -3.14, "MAX": 3.14, "DESCRIPTION": "X-axis rotation for Sierpinski transformation (first pass)." },
    { "NAME": "sierpinski_rotation_y", "TYPE": "float", "DEFAULT": 1.0, "MIN": -3.14, "MAX": 3.14, "DESCRIPTION": "Y-axis rotation for Sierpinski transformation (first pass)." },
    { "NAME": "sierpinski_rotation_z", "TYPE": "float", "DEFAULT": 3.0, "MIN": -3.14, "MAX": 3.14, "DESCRIPTION": "Z-axis rotation for Sierpinski transformation (second pass)." },
    { "NAME": "sierpinski_rotation_w", "TYPE": "float", "DEFAULT": 0.0, "MIN": -3.14, "MAX": 3.14, "DESCRIPTION": "W-axis rotation for Sierpinski transformation (second pass)." },
...
// Sierpinski Octahedron SDF function - directly from your code, adapted for ISF inputs
vec2 sierpinskioctahedron_sdf(vec3 z) {
    // All these variables are implicitly passed from the JSON INPUTS as floats
    // float fractal_power;
    // float sierpinski_rotation_x/y/z/w;
    // float sierpinski_offset_x/y/z;
    // float sierpinski_mouse_toggle;
    // float iterations;

    float scale = fractal_power;

    vec4 rotation = vec4(sierpinski_rotation_x, sierpinski_rotation_y, sierpinski_rotation_z, sierpinski_rotation_w);
    vec3 offset;
       
    if (sierpinski_mouse_toggle > 0.5) { // Check toggle as float
        // Reconstruct mouse-like offset from sliders
        // Original: vec3(9.0 * iMouse.xy / iResolution.xy - (9.0/2.0), 0.8)
        // Assuming sierpinski_offset_x/y are now normalized 0-1 values if toggle is on
        offset = vec3(9.0 * sierpinski_offset_x - 4.5, 9.0 * sierpinski_offset_y - 4.5, 0.8); 
    } else {
        offset = vec3(sierpinski_offset_x, sierpinski_offset_y, sierpinski_offset_z);
    }
    
...
    while (n < int(iterations)) { // Cast iterations to int
        z.xz = vec2(cos(rotation.x) * z.x - sin(rotation.x) * z.z, sin(rotation.x) * z.x + cos(rotation.x) * z.z);
        z.yz = vec2(cos(rotation.y) * z.y - sin(rotation.y) * z.z, sin(rotation.y) * z.y + cos(rotation.y) * z.z);

        z = abs(z);
        if (z.y - z.z < 0.0) z.yz = z.zy;
		if (z.z + z.y < 0.0) z.zy = z.yz; // Original had a typo: z.z + z.y < 0.0, assuming it should be z.y + z.z < 0.0 for symmetry
        
        z = 4.0 * clamp(z, vec3(0.0), vec3(1.0)) - z;

        z = z * scale - offset * (scale - 1.0);

        z.xz = vec2(cos(rotation.z) * z.x - sin(rotation.z) * z.z, sin(rotation.z) * z.x + cos(rotation.z) * z.z);
        z.yz = vec2(cos(rotation.w) * z.y - sin(rotation.w) * z.z, sin(rotation.w) * z.y + cos(rotation.w) * z.z);
...
    return vec2(length(z) * pow(scale, float(-n-3)), orbit_trap).yx; // Swizzle to match original return order
}
...
vec3 ray_marcher(vec3 ro, vec3 rd) {
	...
	for (int i = 0; i < int(max_steps); i++) { // Cast max_steps to int
		vec2 data = sierpinskioctahedron_sdf(ro + rd * dfo);
...
float soft_shadow(vec3 p, vec3 light_pos, float k) {
	...
	for (int i = 0; i < int(shadow_steps); i++) { // Cast shadow_steps to int
		float h = sierpinskioctahedron_sdf(p + rd * t).y;
...
vec3 calculate_normal(vec3 p) {
	...
	return normalize(vec3(
		sierpinskioctahedron_sdf(p + vec3(h, 0.0, 0.0)).y - sierpinskioctahedron_sdf(p - vec3(h, 0.0, 0.0)).y,
		sierpinskioctahedron_sdf(p + vec3(0.0, h, 0.0)).y - sierpinskioctahedron_sdf(p - vec3(0.0, h, 0.0)).y,
		sierpinskioctahedron_sdf(p + vec3(0.0, 0.0, h)).y - sierpinskioctahedron_sdf(p - vec3(0.0, 0.0, h)).y
	));
}
...
float calculate_ambient_occlusion(vec3 p, vec3 normal) {
	...
	for (int i = 0; i < int(ambient_occlusion_steps); i++) { // Cast to int
		...
		float d = sierpinskioctahedron_sdf(sample_point).y;
