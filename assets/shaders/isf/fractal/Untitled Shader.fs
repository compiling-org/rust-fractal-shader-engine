/*{
  "DESCRIPTION": "Converted 'Kaleidoscopic Journey' to ISF. Includes palettes, pulse, shake, geometry morph, color controls.",
  "CATEGORIES": ["Fractal", "Raymarch", "KIFS", "Psychedelic"],
  "INPUTS": [
...
float trap(vec3 p, float time) {
	return length(p.x - 0.5 - 0.5 * sin(time / 10.0));
}
...
float DE(vec3 z, float time, vec3 offset) {
	...
	for (int n = 0; n < Iterations; n++) {
		...
		d = min(d, trap(z, time) * pow(Scale, -float(n + 1)));
	}
	return d;
}

vec3 getNormal(in vec3 pos, float time, vec3 offset) {
