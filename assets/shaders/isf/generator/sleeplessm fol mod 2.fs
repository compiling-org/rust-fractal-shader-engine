...
void main() {
    ...
    for (int i = 0; i < 3; i++) {
        float time2 = time + float(i) * 0.018;
        vec3 hex = coordToHex(tx, Zoom * nsin(time * 0.18), PI * nsin(time * 0.05));
        vec3 cell = hexToCell(hex, 1.1 + float(i) * 0.5);
        float value = nsin(hexToFloat(cell, nsin(time2 * 1.2)) * WaveFrequency * nsin(time * 0.6) + time2);
        rgb[i] += pow(value, 2.4) * vec3(0.9, 0.6, 1.3)[i];
    }

    // Apply brightness and color intensity
    rgb *= Brightness * ColorIntensity;

    gl_FragColor = vec4(rgb * 1.9, 1.0);
}