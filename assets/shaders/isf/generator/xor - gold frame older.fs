...
vec3 getLightPosition(float current_time) {
    return vec3(cos(current_time * light_orbit_speed) - 1.0, sin(current_time * light_orbit_speed), cos(current_time * light_orbit_speed * 0.5)) * light_orbit_radius - vec3(current_time * light_scroll_speed, 0.0, 0.0);
}
...
vec4 raymarch(vec3 p, vec3 d) {
    ...
    for (int i = 0; i < int(max_raymarch_steps); i++) {
        S_accum = model(P_curr);
        T_total += S_accum;
        P_curr += D_norm * S_accum;
        if ((T_total > max_raymarch_distance) || (S_accum < surface_precision)) break;
    }
    return vec4(P_curr, min(T_total / max_raymarch_distance, 1.0));
}
...
vec3 color1(vec3 p, vec3 n, float current_time) {
    vec3 C = vec3(base_color_r, base_color_g, base_color_b);
    vec3 light_pos = getLightPosition(current_time);
    ...
    total_lighting *= (1.0 + sin(current_time * color_pulse_speed) * color_pulse_amplitude);

    vec3 palette_color = getPal(int(color_palette_type), p.x * 0.1 + current_time * palette_time_multiplier);
    C *= palette_color;
...
vec3 color2(vec3 p, vec3 d, float current_time) {
    vec3 N = normal(p);
    vec3 C = color1(p, N, current_time);
...
// Camera Variables
void camera(out vec3 P_out, out vec3 D_out, out vec3 X_out, out vec3 Y_out, out vec3 Z_out, float current_time) {
    vec2 A_angles;
    float is_mouse_active = 0.0;

    if (camera_control_type > 0.5) {
        A_angles = (vec2(camera_yaw, camera_pitch) - 0.5) * vec2(TAU, PI);
        is_mouse_active = 1.0;
    } else {
        A_angles = (vec2(0.5, 0.5) - vec2(0.5,0.5)) * vec2(TAU, PI);
        is_mouse_active = 0.0;
    }
    ...
    P_out = vec3(-current_time * camera_scroll_speed, 0.0, 0.0) + camera_offset_forward_factor * F;
    D_out = -F;

    X_out = normalize(D_out);
    Y_out = normalize(cross(X_out, vec3(0.0, 0.0, 1.0)));
    Z_out = cross(X_out, Y_out);
}
...
void main() {
    // Moved T calculation into main()
    float T_current = TIME * global_speed;

    vec3 P_cam, D_cam, X_cam, Y_cam, Z_cam;
    camera(P_cam, D_cam, X_cam, Y_cam, Z_cam, T_current);
    ...
    if (shake_strength > 0.001) {
        vec2 shake_offset = vec2(
            sin(T_current * shake_frequency * 1.0 + hash11(1.0)),
            cos(T_current * shake_frequency * 1.1 + hash11(2.0))
        ) * shake_strength * 0.05; 
        UV += shake_offset; 
    }
    ...
    vec4 M_hit_data = raymarch(P_cam, D_cam);
    vec3 final_color = vec3(0.01, 0.01, 0.01);

    if (M_hit_data.w < 0.99) {
        final_color += max(color2(M_hit_data.xyz, D_cam, T_current) * sqrt(1.0 - M_hit_data.w), 0.0);
    }
    
    final_color += exp2(-length(cross(D_cam, getLightPosition(T_current) - P_cam))) * light_halo_strength;

    if (glitch_strength > 0.001) {
        float g_time = T_current * glitch_frequency;
