...
// Replaces the mainImage from Buffer A
vec4 renderFractalScene(vec2 fragCoord, float current_time_buffer_pass) {
    ...
    const int NB_FRAMES = 17; // Adjust based on the actual number of elements in your arrays

    // Original array data (defined as constants)
    const float camx_arr[NB_FRAMES] = float[] ( .2351, 1.2351, 1.2351, 1., .2, .41, .5,.084,.145,  3.04,.12, .44,.44,.416, -1.404, .21, .2351);
    const float camy_arr[NB_FRAMES] = float[] (-.094,  .35,     .28,  .38,  .04, .11, .35,.0614,.418, 1.,-.96, .67,.8,.0, -1., -.06,-.094);
    const float camz_arr[NB_FRAMES] = float[] ( .608,  .608,    .35,    .3608, -.03, .48, .47,0.201,.05,.28,.3, 1.445,1.,1.4, 2.019, .508,.608);

    const float lookx_arr[NB_FRAMES] = float[] (-.73, -.627, -1., -.3, -1., -.72, -.67,-.5,-.07,-.67,-.27, -.35,-.35,-.775, .08, -.727, -.727);
    const float looky_arr[NB_FRAMES] = float[] (-.364, -.2,   -.2,  -.2,  0., -.39, -.56,-.37,-.96,-.74,-.94, -.35,-.35,-.1, .83,-.364, -.364);
    const float lookz_arr[NB_FRAMES] = float[] (-.582, -.582, -.5, -.35, -.0, -.58, -.48,-.79,-.25,.06,-.18, -.87,-.87,.23, .55, -.582, -.582);

    const float minsx_arr[NB_FRAMES] = float[] (-.3252,-.3252,-.3252,-.3252,-.3252,-.3252, -1.05,-1.05,-1.21,-1.22,-1.04,-0.737,-.62,-10., -.653, -.653, -.3252);
    const float minsy_arr[NB_FRAMES] = float[] (-.7862,-.7862,-.7862,-.7862,-.7862,-.7862, -1.05,-1.05,-.954,-1.17,-.79,-0.73,-.71,-.75, -2., -2., -.7862);
    const float minsz_arr[NB_FRAMES] = float[] (-.0948,-.0948,-.0948,-.0948,-.0948,-.0948,-0.0001,-0.0001,-.0001,-.032,-.126,-1.23,-.85,-.787, -.822, -1.073, -.0948);
    const float minsw_arr[NB_FRAMES] = float[] ( .69, .69, .69, .69, .69, .678, .7,.73,1.684,1.49,.833, .627,.77,.826, 1.8976, 1.8899, .69);

    const float maxsx_arr[NB_FRAMES] = float[] ( .35,.3457,.3457,.3457,.3457, .3457, 1.05,1.05,.39,.85,.3457,.73,.72,5., .888, .735, .35);
    const float maxsy_arr[NB_FRAMES] = float[] (1.,1.0218,1.0218,1.0218,1.0218,1.0218,1.0218,1.05,1.05,.65,.65,1.0218,0.73,.74,1.67, .1665, 1.);
    const float maxsz_arr[NB_FRAMES] = float[] (1.22,1.2215,1.2215,1.2215,1.2215,1.2215,1.2215,1.27,1.4,1.27,1.27,1.2215,.73,.74,.775, 1.2676, 1.22);
    const float maxsw_arr[NB_FRAMES] = float[] ( .84, .84, .84, .84, .84, .9834,.9834,.95,.93,2.74,1.23,.9834, .8335,.14,1.172, .7798, .84);


    float t_interp_base = .1*current_time_buffer_pass;
    float kt = smoothstep(0.,1.,fract(t_interp_base));
    
    int i0 = int(t_interp_base) % NB_FRAMES;
    int i1 = (i0 + 1) % NB_FRAMES; // Wrap around for i1

    // Base camera and look-at vectors from original keyframes
    vec3 ro_base = mix(vec3(camx_arr[i0],camy_arr[i0],camz_arr[i0]), vec3(camx_arr[i1],camy_arr[i1],camz_arr[i1]), kt);
    vec3 ww_base = mix(vec3(lookx_arr[i0],looky_arr[i0],lookz_arr[i0]), vec3(lookx_arr[i1],looky_arr[i1],lookz_arr[i1]), kt);
    ...
    mins_global = mix(vec4(minsx_arr[i0],minsy_arr[i0],minsz_arr[i0],minsw_arr[i0]), vec4(minsx_arr[i1],minsy_arr[i1],minsz_arr[i1],minsw_arr[i1]), kt);
    maxs_global = mix(vec4(maxsx_arr[i0],maxsy_arr[i0],maxsz_arr[i0],maxsw_arr[i0]), vec4(maxsx_arr[i1],maxsy_arr[i1],maxsz_arr[i1],maxsw_arr[i1]), kt);
...
void drawText(float time_param, vec2 frag_coord_param, inout vec4 color) {
    if (textEnabled > 0.5) { // Control text with ISF input
        ...
        float ofs = floor(uv_char.x), x = 1.0, nr = 0.;
...
void main()
{
    ...
    if (dofEnabled > 0.5) {
        ...
        const float deph_arr[NB_DOF_FRAMES] = float[] ( 1.,.65,.6,.4,.2,.4,.65,.11,.13,1.3,.49,1.2,1.2,.5,.65,.45,1.);
        
        float t_dof = .1 * current_time_total;
        float kt_dof = smoothstep(0.,1.,fract(t_dof));
        int i0_dof = int(t_dof) % NB_DOF_FRAMES;
        int i1_dof = (i0_dof + 1) % NB_DOF_FRAMES;

        float fdist = mix(deph_arr[i0_dof], deph_arr[i1_dof], kt_dof);
