/*
{
  ...
  "INPUTS": [
    ...
    { "NAME": "PaletteBlend", "TYPE": "float", "DEFAULT": 1.0, "MIN": 0.0, "MAX": 1.0, "LABEL": "Palette Intensity" }, 
    { "NAME": "GlobalBrightness", "TYPE": "float", "MIN": 0.0, "MAX": 5.0, "DEFAULT": 1.8, "LABEL": "Global Brightness" },

    { "NAME": "CameraZOffset", "TYPE": "float", "MIN": -10.0, "MAX": 10.0, "DEFAULT": -10.0, "LABEL": "Camera Z Offset" },
    { "NAME": "CameraZAnimationSpeed", "TYPE": "float", "MIN": 0.0, "MAX": 0.5, "DEFAULT": 0.05, "LABEL": "Camera Z Anim Speed" },
    { "NAME": "CameraDepthEffect", "TYPE": "float", "MIN": 0.1, "MAX": 2.0, "DEFAULT": 0.5, "LABEL": "Camera Depth Effect" },
    { "NAME": "CameraDepthPulseFreq", "TYPE": "float", "MIN": 0.0, "MAX": 10.0, "DEFAULT": 0.25, "LABEL": "Camera Depth Pulse Freq" },
    { "NAME": "CameraDepthPulseAmplitude", "TYPE": "float", "MIN": 0.0, "MAX": 1.0, "DEFAULT": 0.2, "LABEL": "Camera Depth Pulse Amp" },
    { "NAME": "CameraXYOffset", "TYPE": "point2D", "DEFAULT": [0.0, 0.0], "LABEL": "Camera XY Offset" },

    { "NAME": "FractalIterations", "TYPE": "float", "MIN": 1.0, "MAX": 150.0, "DEFAULT": 150.0, "LABEL": "Fractal Iterations" },
    { "NAME": "FractalInitialScaleMorphFactor", "TYPE": "float", "MIN": 0.1, "MAX": 2.0, "DEFAULT": 2.0, "LABEL": "Initial Scale/Morph" },
    { "NAME": "FractalFoldingAnimationStrength", "TYPE": "float", "MIN": 0.0, "MAX": 0.1, "DEFAULT": 0.1, "LABEL": "Folding Anim Strength" },
    { "NAME": "FractalInnerMorphStrength", "TYPE": "float", "MIN": 0.1, "MAX": 2.0, "DEFAULT": 2.0, "LABEL": "Inner Morph Strength" },
    { "NAME": "FractalIterationScaleFactor", "TYPE": "float", "MIN": 0.5, "MAX": 5.0, "DEFAULT": 1.9, "LABEL": "Iteration Scale Factor" },
    { "NAME": "FractalIterationClampMin", "TYPE": "float", "MIN": 0.01, "MAX": 1.0, "DEFAULT": 0.0, "LABEL": "Iteration Clamp Min" },
    { "NAME": "FractalIterationClampMax", "TYPE": "float", "MIN": 1.0, "MAX": 5.0, "DEFAULT": 5.0, "LABEL": "Iteration Clamp Max" },
    { "NAME": "SurfaceHitThreshold", "TYPE": "float", "MIN": 0.01, "MAX": 1.0, "DEFAULT": 0.6, "LABEL": "Surface Hit Threshold" },
    { "NAME": "ColorFadeFactor", "TYPE": "float", "MIN": 0.001, "MAX": 0.1, "DEFAULT": 0.1, "LABEL": "Color Fade Factor" },

    { "NAME": "FlashFreqRay", "TYPE": "float", "MIN": 0.0, "MAX": 20.0, "DEFAULT": 5.0, "LABEL": "Flash Freq (Ray)" },
    { "NAME": "FlashAmpRay", "TYPE": "float", "MIN": 0.0, "MAX": 1.0, "DEFAULT": 1.0, "LABEL": "Flash Amp (Ray)" },
    { "NAME": "FlashFreqIteration", "TYPE": "float", "MIN": 0.0, "MAX": 20.0, "DEFAULT": 20.0, "LABEL": "Flash Freq (Iter)" },
    { "NAME": "FlashAmpIteration", "TYPE": "float", "MIN": 0.0, "MAX": 1.0, "DEFAULT": 1.0, "LABEL": "Flash Amp (Iter)" }
  ],
  "ISFVSN": "2"
}
*/
...
vec3 trippyPalette(float t_val, float time_val, float iter_val, float pulse_freq, float pulse_amp) {
    ...
    // Modulate hue frequencies by pulse_freq
    float hue_base = mod(t_val * 0.7 + time_val * (0.6 + pulse_freq * 0.1) + iter_val * 0.25, 1.0); // Increased impact of pulse_freq
    float hue_shift1 = mod(t_val * 1.1 + time_val * (0.9 + pulse_freq * 0.1) + iter_val * 0.35 + 0.33, 1.0); // Increased impact of pulse_freq
    float hue_shift2 = mod(t_val * 1.5 + time_val * (1.2 + pulse_freq * 0.1) + iter_val * 0.45 + 0.66, 1.0); // Increased impact of pulse_freq
    ...
    // Modulate frequency and amplitude by pulse_freq and pulse_amp
    float sat_pulse = (0.3 + 0.7 * fract(sin(time_val * (15.0 + pulse_freq * 1.0) + t_val * 0.8) * 500.0)) * (1.0 + pulse_amp * 1.0); // Increased impact of pulse_freq and pulse_amp
    float val_pulse = (0.1 + 0.9 * fract(cos(time_val * (13.0 + pulse_freq * 1.0) + iter_val * 0.6) * 500.0)) * (1.0 + pulse_amp * 1.0); // Increased impact of pulse_freq and pulse_amp

    // Clamp saturation and value pulses to avoid over-saturation/brightness
    sat_pulse = clamp(sat_pulse, 0.0, 1.0);
    val_pulse = clamp(val_pulse, 0.0, 1.0);
...
void main() {
    ...
    for(int i_ray_int = 0; i_ray_int < 200; i_ray_int++) { // Use fixed int for loop, then cast to float for calculations
        ...
        p_transformed_current_iter.xy *= rot(0.5 * PI * sin(scaledTime * OverallRotationSpeed)); // Apply rotation here
        ...
        float d_step_size = length(p_transformed_current_iter) / s_accumulated_scale; // Using 3D length for more reliable stepping
        ...
        if(d_step_size < 0.001) { // Hit detected (very close to surface)
            ...
            vec3 psychedelic_color = trippyPalette(t, scaledTime, i_ray, ColorPulseFreq, ColorPulseAmp); 
    ...
    if(t >= 100.0) { 
        vec3 fog_color = trippyPalette(t * 0.05, scaledTime, 0.0, ColorPulseFreq, ColorPulseAmp) * 0.1 * PaletteBlend; 
    ...
    if (length(col_final) > 0.001) { 
        ...
        vec3 al_base = trippyPalette(scaledTime * 0.4, scaledTime, 0.0, ColorPulseFreq, ColorPulseAmp) * PaletteBlend * 2.5; 
        vec3 al = al_base;
        ...
        vec3 fog_color_dynamic = trippyPalette(t * 0.04, scaledTime, 0.0, ColorPulseFreq, ColorPulseAmp) * 0.4 * PaletteBlend; 
    ...
    // The global Color Pulse is applied as a final effect (overall brightness pulse)
