...
vec2 map(vec3 p){
    ...
    // mod
    float mm = sin(TIME*speed*0.5 + g.z*0.5 + p.z);
    modu = (mm/sqrt(0.02 + mm*mm ))*0.5 + 0.5;
    float mmB = sin(TIME*speed*1.25 + g.z*0.25 + p.z*0.8 + p.y);
        ...
        if (d.y < 10.){
            glow += mix(
                mix(
                    ...
                    exp(-d.x*0.01)*pal(2.8,1.4,vec3(5.8,2.4+modu*0.2,0.8+ sin(p.y + TIME*speed)*.09), 6.9 +modu*0.2 + sin(p.z)*.5,0.5 + t*0.5)*2.,
                    moduB
