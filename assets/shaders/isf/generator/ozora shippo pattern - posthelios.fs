/*
{
    ...
    "DESCRIPTION": "An ISF conversion of a Shadertoy fractal, featuring repeating 3D structures with dynamic motion and vibrant colors. Includes comprehensive controls for animation speed, zoom, camera, fractal geometry, color palettes, and post-processing effects like glitch, shake, brightness, saturation, and contrast.",
    ...
    "INPUTS": [
        ...
        { "NAME": "z_scroll_speed", "TYPE": "float", "DEFAULT": 6.5, "MIN": 0.0, "MAX": 15.0, "DESCRIPTION": "Speed of Z-axis scrolling/translation." },
        ...
        { "NAME": "offset_x_tan_speed_mult", "TYPE": "float", "DEFAULT": 0.5, "MIN": 0.0, "MAX": 2.0, "DESCRIPTION": "Tangent speed multiplier for X offset (adds chaotic motion)." },
...
void main() {
    ...
    for(int j = 0; j < int(raymarch_iterations); j++) {
        ...
        p.z += current_time * z_scroll_speed; // p.z+=iTime*6.5;
        ...
        for(int k = 0; k < int(fractal_repetitions); k++) { // Loop 8 times (tunable)
            ...
            // p=abs(p)*e - vec3(5.+cos(iTime*.3+.5*tan(iTime*.3))*3.,120,8.+cos(iTime*.5)*5.);
            // Apply scale and complex dynamic offset
            p = abs(p) * e_val - 
                vec3(
                    offset_x_base + cos(current_time * offset_x_osc_speed + offset_x_tan_speed_mult * tan(current_time * offset_x_osc_speed)) * offset_x_osc_mult, // Tunable
        ...
        e_val = length(p.yz) / s_val; // Use yz plane distance for this fractal's specific logic
