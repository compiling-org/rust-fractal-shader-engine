/*
{
    ...
    "INPUTS": [
        ...
        },
        {
            ...
            "MAX": 50.0,
            "STEP": 0.1,
            "DESCRIPTION": "Speed/frequency of the camera shake effect."
        },
        {
            "NAME": "fractalDepth",
            "TYPE": "float",
...
#define ORB_RADIUS .5 // Defines the radius of the light orb
...
// --- Color Utility Functions (Moved to top for correct definition order) ---
...
// --- Fractal Function ---
// Generates a spatially varied (multi-colored) but STATIC base color for the fractal.
// All temporal color animation is handled by applyColorPalette.
float fractal(vec3 p, out vec3 fractal_color_out, float iterations_float, float morphA, float morphB, float morphC){
  vec3 p_initial_for_color = p; // Store initial P for base color mapping

  for (int i = 0; i < int(iterations_float); i++) {
    float n = abs(p.x * p.y * p.z);
    p = abs(p) / clamp(n, morphA, morphB) - morphC; // Tunable morphing
  }
  
  // Generate a vibrant, spatially varied base color from the *iterated* point 'p'
  // This ties the color directly to the fractal's complex geometry.
  float hue_base = fract(p.x * 0.11 + p.y * 0.08 + p.z * 0.05); // Use all components for hue
  float saturation_base = 0.7 + 0.3 * abs(sin(p.y * 0.2)); // High saturation with spatial variation
  float value_base = 0.6 + 0.4 * abs(cos(p.x * 0.15 + p.z * 0.07)); // Good brightness with spatial variation

  fractal_color_out = hsv2rgb(vec3(hue_base, saturation_base, value_base));

  // The distance calculation should reflect the geometry well.
  // Using length of final P is common for Mandelbulb/similar fractals.
  return length(p) * 0.1; 
}
...
float scene(vec3 p, float time_val, out vec3 rgb) {
    ...
    if (orb == hit) { // If we hit the orb
        fractal(vec3(tun.xy, p.z), rgb, fractalDepth, fractalMorphA, fractalMorphB, fractalMorphC); // Sets RGB color via 'out' parameter
        rgb *= vec3(.05, 1.5, .05); // Orb color - green/yellowish tint applied to multi-colored fractal base
    } else { // If we hit the tunnel walls
        fractal(vec3(tun.xy, sin(p.z)), rgb, fractalDepth, fractalMorphA, fractalMorphB, fractalMorphC); // Sets RGB color via 'out' parameter
    }
    return hit;
}
...
float raymarch(vec3 ro, vec3 rd, float time_val, out vec3 rgb) {
  ...
  for(float i = 0.; i < MAX_STEPS; i++) {
    vec3 p = ro + rd * dist;
    float step = scene(p, time_val, rgb); // RGB is filled by scene() which calls fractal()
    dist += step;
...
// This function now expects an already multi-colored 'color_in' from the fractal.
vec3 applyColorPalette(vec3 color_in, float iTime_scaled) {
    ...
    // These now *add* to or *modulate* the existing hsv values, not replace them.
    ...
    hsv.x = mod(hsv.x + huePulse, 1.0); // Additive hue shift for pulsing
    ...
    // Each palette now applies *additional* transformations to the already varied HSV input.
    // The goal is to shift the overall feel of the colors while preserving internal variations.
    if (palette == 1) { // Psychedelic Dream (Purples, Cyans, Pinks - more intense)
        hsv.x = mod(hsv.x + 0.6 + sin(iTime_scaled * 0.08) * 0.05, 1.0); // Shift towards purples/pinks
        hsv.y = clamp(hsv.y * 1.5 + 0.2, 0.0, 1.0); // Higher saturation baseline
        hsv.z = clamp(hsv.z * 1.3, 0.0, 1.0); // Brighter
    } else if (palette == 2) { // Electric Neon (Greens, Yellows, Bright Blues - sharper)
        hsv.x = mod(hsv.x + 0.25 + cos(iTime_scaled * 0.1) * 0.07, 1.0); // Shift towards greens/yellows/blues
        hsv.y = clamp(hsv.y * 1.8 + 0.15, 0.0, 1.0); // Very high saturation
        hsv.z = clamp(hsv.z * 1.4, 0.0, 1.0); // Very bright
    } else if (palette == 3) { // Fiery Abyss (Deep Reds, Oranges, Dark Purples - more dynamic)
        hsv.x = mod(hsv.x + 0.0 + sin(iTime_scaled * 0.06) * 0.04, 1.0); // Shift towards reds/oranges
        hsv.y = clamp(hsv.y * 1.6 + 0.25, 0.0, 1.0); 
        hsv.z = clamp(hsv.z * 1.0, 0.0, 1.0); // Keep deep tones
    } else if (palette == 4) { // Oceanic Tranquility (Blues, Greens, Aquas - deeper, more fluid)
        hsv.x = mod(hsv.x + 0.45 + cos(iTime_scaled * 0.09) * 0.06, 1.0); // Shift towards blues/greens
        hsv.y = clamp(hsv.y * 1.7 + 0.2, 0.0, 1.0);
        hsv.z = clamp(hsv.z * 1.2, 0.0, 1.0);
    } else if (palette == 5) { // Galactic Glitch (Vibrant, glitchy)
        hsv.x = mod(hsv.x + 0.75 + sin(iTime_scaled * 0.12) * 0.08, 1.0); // Erratic hue shifts across spectrum
        hsv.y = clamp(hsv.y * 1.6 + 0.1, 0.0, 1.0); // Good saturation
        hsv.z = clamp(hsv.z * 1.3, 0.0, 1.0);
    } else if (palette == 6) { // Rainbow Pulse (more intense and full spectrum)
        hsv.x = mod(hsv.x + iTime_scaled * 0.2, 1.0); // Faster hue rotation for full rainbow
        hsv.y = clamp(hsv.y * 2.0 + 0.3, 0.0, 1.0); // Max saturation
        hsv.z = clamp(hsv.z * 1.4, 0.0, 1.0); // Brighter
    } else if (palette == 7) { // High Contrast Burst (vivid)
        hsv.x = mod(hsv.x + sin(iTime_scaled * 0.5) * 0.1 + cos(iTime_scaled * 0.3) * 0.15, 1.0); // Rapid, broad hue shifts
        hsv.y = clamp(hsv.y * 0.8 + 0.5, 0.0, 1.0); // Ensure high saturation for bursts
        hsv.z = clamp(hsv.z * 1.8, 0.0, 1.0); // Boost value for strong contrast and brightness
    }
...
void main() {
    ...
    if (shakeIntensity > 0.0) {
        float shakeTime = totalTime * shakeSpeed;
        vec2 shakeOffset = vec2(
            sin(shakeTime * 12.34 + 0.1) * cos(shakeTime * 7.89 + 0.5),
            cos(shakeTime * 9.87 + 0.3) * sin(shakeTime * 5.43 + 0.7)
        ) * shakeIntensity;
        uv_modified += shakeOffset;
    }
	...
	float d = raymarch(ro, rd, totalTime, rgb); // rgb will be filled by scene() with a multi-colored base
    ...
    // Add an emissive glow for the orb itself
    float dist_to_orb_center = length(lightPosition(p, totalTime));
    if (d < MAX_DIST && dist_to_orb_center < ORB_RADIUS + 0.2) { // If we hit something and are near the orb
        float orb_glow_strength = smoothstep(ORB_RADIUS + 0.2, ORB_RADIUS * 0.1, dist_to_orb_center);
        rgb += vec3(0.0, 0.8, 0.0) * orb_glow_strength * 2.0; // Strong green glow for the orb
    }

    // Apply color palette and pulsing (this now works on a truly multi-colored, animated base)
