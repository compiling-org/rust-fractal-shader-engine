/*
{
    ...
    "DESCRIPTION": "Automatically converted from https://www.shadertoy.com/view/DlKXzd by LydianLights. Hexagons are good. Enhanced with tunable parameters for animation speed, zoom, color morphing, and interactivity.",
    "IMPORTED": {},
    "INPUTS": [
        {
            "NAME": "Speed",
...
vec3 mainPass(in vec2 fragCoord) {
    ...
    // Loop to generate multiple layers of hexagons
    for (float i = 0.0; i < 5.0; i++) {
        h = getHex(1.1 * ROOT_3 * h * rot).xy;

        float d = hexDist(h);
        d = 2.0 * d * pow(0.2, d0);
        d = 0.5 * sin(4.0 * d - 0.5 * TIME * Speed + i * 2.0 * PI / 7.0); // Use Speed parameter
        d = 0.04 / d;
        d = pow(d, 2.0);

        // Use ColorSeed to randomize the color palette
        vec3 c = d * palette(length(h0) + 0.3 * i, ColorSeed);
        color += c;
    }
