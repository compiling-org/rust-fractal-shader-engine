/*
{
  "DESCRIPTION": "Circuit fractal on a KIFS structure with color pulse, palette, and motion controls.",
  ...
  "INPUTS": [
    ...
    { "NAME": "camMotion", "TYPE": "float", "DEFAULT": 3.0, "MIN": 0.0, "MAX": 10.0, "LABEL": "Camera Motion Intensity" },
...
vec3 palette(float t, int mode) {
    ...
    if (mode == 2) return vec3(1.2, 0.3, 0.1) * sin(2.0 * 3.14 * t);                        // Lava
    if (mode == 3) return vec3(0.2, 0.6, 1.0) * cos(2.5 * 3.14 * t);                        // Ice
