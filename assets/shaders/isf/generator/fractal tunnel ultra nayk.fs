/*{
  "DESCRIPTION": "Original tunnel preserved with spatially varying multicolor palettes and advanced fractal controls",
  ...
  "INPUTS": [
    ...
    { "NAME": "fractalScale",  "TYPE": "float", "DEFAULT": 2.2, "MIN": 0.5, "MAX": 5.0 },
    { "NAME": "fractalWarp",   "TYPE": "float", "DEFAULT": 0.0, "MIN": 0.0, "MAX": 3.0 },
    { "NAME": "fractalRepeat", "TYPE": "float", "DEFAULT": 6.0, "MIN": 1.0, "MAX": 10.0 },
...
vec3 getPalette(float t, vec3 pos, int i) {
  float spatial = dot(pos.xy, vec2(0.2, 0.3)) + pos.z * 0.1;
  float val = t + spatial;

  if (i == 0) return 0.5 + 0.5 * cos(TAU * (vec3(0.0, 0.33, 0.67) + val));
  if (i == 1) return 0.6 + 0.4 * sin(val * vec3(1.5, 2.2, 3.1));
  if (i == 2) return mix(vec3(0.2,0.9,0.5), vec3(1.0,0.3,0.8), sin(val)*0.5+0.5);
  if (i == 3) return fract(val * vec3(1.5, 1.1, 0.9));
  if (i == 4) return vec3(sin(val * 3.0), cos(val * 2.0), sin(val * 4.5)) * 0.5 + 0.5;
  if (i == 5) return vec3(val * 0.15, sin(val), 1.0 - cos(val)) * 0.5 + 0.5;
  return 0.5 + 0.5 * sin(val + vec3(0.2, 0.5, 0.8));
}
...
void main() {
  ...
  for (float i = 0.0; i < 110.0; i++) {
    ...
    for (int j = 0; j < int(fractalRepeat); j++) {
      p = abs(p - vec3(1.0)) - vec3(1.1, 1.3, 1.0);
      p += sin(p * fractalWarp); // distortion
      s *= e = fractalScale / clamp(dot(p, p), 0.5, 2.3);
      p = abs(p) * e;
    }
    ...
    vec3 base = getPalette(log(s) * 0.3 + pulse, p, int(paletteIndex));
    col += mix(vec3(0.1, 0.2, 1.0), base, 0.6) * 0.02 * exp(-0.3 * i * i * e);
  }

  uv *= 2.0 * (cos(TIME * 2.0 * speed) - 2.5);
  vec3 starCol = happy_star(uv, anim) * getPalette(anim * 0.5 + TIME * 0.2, vec3(uv, 0.0), int(paletteIndex)) * 1.15;
